...
export async function POST(
  ...
  if (lobby.status === 'closed') {
    const { data: existing } = await supabase.from('results').select('*').eq('lobby_id', params.id).single();
  ...
  const { data: rows, error: rerr } = await supabase
    .from('rankings')
    .select('user_id,candidate_id,position')
    .eq('lobby_id', params.id);
  if (rerr) return NextResponse.json({ error: rerr.message }, { status: 500 });

  // build rankings-by-user; require full ballots
  ...
  for (const [uid, posMap] of Object.entries(temp)) {
    if (Object.keys(posMap).length !== candidateIds.length) continue; // only full ballots
  ...
  const { data: result, error: iErr } = await supabase
    .from('results')
    .upsert({
      lobby_id: params.id,
      method: 'borda',
  ...
  await supabase.from('lobbies').update({ status: 'closed', closed_at: new Date().toISOString() }).eq('id', params.id);
